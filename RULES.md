...
- **English-only server strings:** the guard `ask` ("I did not change the schedule: ...") and its `explanation` ("Nothing was changed.") are server-owned English, like every `_build_resolution_ask` string, regardless of the user's language — the fourth such string for any future i18n pass, alongside the feature 0084 category ask below.
