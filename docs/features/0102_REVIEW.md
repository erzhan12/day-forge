...
- **New Node-adapter prod entries:** axios 1.18.1 adds four prod-scope Node-native packages (`https-proxy-agent@5.0.1`, `agent-base@6.0.2`, `debug@4.4.3`, `ms@2.1.3`). Node-adapter-only, tree-shaken from the browser bundle (Vite uses axios' XHR adapter). If any ever attracts a high/critical advisory, the new `npm audit --omit=dev --audit-level=high` CI gate catches it before it ships.
- **Bundle size:** baseline `dist/assets/app.js` = 374,089 B → PR = 384,637 B (**+10.5 KB / +2.8%**), attributable to axios core growth across 4 minor versions, not the tree-shaken Node transitives. Acceptable for a security bump.
