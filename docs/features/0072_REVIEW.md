...
| 2 | P2 | `math.isfinite()` raises `OverflowError` on an over-large JSON integer → 500 instead of structured 400 | **FIXED** — dropped `isfinite` (and the now-unused `import math`); the plain range comparison rejects NaN/±Inf/over-large-int without raising. Tests add `10**400` and `-(10**400)` → 400. |
