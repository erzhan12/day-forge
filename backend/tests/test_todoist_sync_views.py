...
class TestConnectRateLimit(ConnectRateLimitContract):
    ...
    SLUG = "todoist"
    SETTINGS_ATTR = "TODOIST_CONNECT_RATE_LIMIT_PER_HOUR"
    MOCK_TARGET = "todoist_sync.service.verify_credentials"
    AUTH_ERROR = service.TodoistAuthError
    valid_body = {"token": "valid-token"}
    oversized_body = {"token": "x" * 200_000}
    EXPECTED_VERIFY_ARGS = ("valid-token",)
...
class TestCacheInvalidation:
    ...
    def test_invalidate_tasks_does_not_resurrect_deleted_account(self, account):
        """A stale in-memory account must not be re-inserted after DELETE."""
        from todoist_sync import cache as todoist_cache

        stale_row = TodoistAccount.objects.get(pk=account.pk)
        pk = stale_row.pk
        TodoistAccount.objects.filter(pk=pk).delete()

        todoist_cache.invalidate_tasks(stale_row)

        assert not TodoistAccount.objects.filter(pk=pk).exists()
