...
def error_locmem_cache_with_ai_in_production(app_configs, **kwargs):
    ...
    by any client whose connections round-robin across workers. Gated
    on ``DEBUG=False`` (dev runs stay silent) and on ``LLM_API_KEY``
    being set (AI features actually in use).
    """
    errors = []
    if settings.DEBUG:
        return errors
    ...
    if backend.endswith(".LocMemCache"):
        errors.append(
            Error(
                ...
                "LLM_API_KEY is set and DEBUG=False. LocMemCache is "
                "per-process, so the per-user rate limit becomes "
