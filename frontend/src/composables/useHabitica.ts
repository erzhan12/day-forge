// Schedule-page composable for Habitica tasks (read + complete + live refresh).
//
...
// Habitica panel gates purely on `connected`, so `fetchTasks` must own
// `connected` on every terminal path. `GET /api/habitica/tasks/<date>/`
...
function defaultState(): HabiticaState {
  return {
    tasks: [],
    loading: false,
    error: null,
...
export function useHabitica() {
  const state = reactive<HabiticaState>(defaultState())
  ...
  function statusToMessage(status: number | undefined): string | null {
    switch (status) {
      case 401:
        return "Habitica credentials invalid. Reconnect in Settings."
      case 502:
      case 504:
        return "Habitica service unavailable. Try again later."
      case 503:
  ...
  async function _fetchTasks(
    ...
    const tasksUrl = `/api/habitica/tasks/${date}/${query}`
    ...
    state.loading = false
    if (result.ok) {
      const tasks = (result.data?.tasks as HabiticaTask[]) ?? []
      state.tasks = tasks
    ...
    // `HabiticaAccount.DoesNotExist`, and a `401/500/502/504` reaches the
  ...
  async function completeTask(taskId: string): Promise<void> {
    ...
    const result = await requestJson(
      `/api/habitica/tasks/${taskId}/complete/`,
      "POST",
      undefined,
    )
  ...
  async function fetchAccountStatus(): Promise<void> {
    ...
    if (result.ok) {
      const body = result.data as HabiticaAccountStatus | undefined
