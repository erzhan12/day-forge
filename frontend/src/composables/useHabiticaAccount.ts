// Settings-page composable for the Habitica account.
//
...
import type { HabiticaAccountStatus } from "../types/habitica"

interface AccountState {
  status: HabiticaAccountStatus | null
  loading: boolean
  error: string | null
}
...
export function useHabiticaAccount() {
  ...
  async function connect(payload: {
    ...
    try {
      let result
      try {
        result = await requestJson("/api/habitica/account/", "POST", payload, {
      ...
      state.loading = false
      if (result.ok) {
        state.status = result.data as unknown as HabiticaAccountStatus
        state.error = null
      } else {
